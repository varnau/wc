...
// 1) secuencia Fasta (cromosoma)
// 2) fichero con READS (una por linea) -->  sigo con los FASTQ!!
// Usar valor de K alto. 
//==============================================================================
...
void main(int argc, char *argv[]) {
 ...
 //----------------------------------------------------------
  ...
  strcpy(ficha, argv[1]);  //------ FICHERO entrada  1  : FILE-1.FASTA
 
  if ( (f_fa=fopen(ficha,"r"))!=NULL)  //-->>  Pruebo con el nombre tal cual
    printf ("\n File_FASTA : %s", ficha);
  else {
    strcpy(ficha, argv[1]);
    strcat(ficha, ".fa");                  //  o Fichero.FA
    if ( (f_fa=fopen(ficha,"r"))==NULL) {
  ...
  fseek(f_fa, 0, SEEK_END);       //------- NOU: per saber el tamany del fitxer
  nbytes1 = ftell(f_fa);
  printf("\t  --> Size = %ld bytes\n", nbytes1);
  fseek(f_fa, 0, SEEK_SET);



  strcpy(ficha, argv[2]);  //------ FICHERO entrada  2  : FILE-2.READS

  if ( (f_in=fopen(ficha,"r"))!=NULL)  //-->>  Pruebo con el nombre tal cual
    printf ("\n File_reads : %s", ficha);
  else {
      printf ("\n Don't find  the File-2 %s.\n", argv[2]);
      exit(0);
    }
  ...
  strcpy(ficha, argv[3]);  //------ FICHERO salida
  f_out=fopen(ficha,"w");
  printf ("\n File_OUT: %s", ficha);

  //============    KK : tamanyo de palabra   ==========================

  WORD = atoi(argv [4]);     //**************  Tamaño de palabra    *******
  if ( (WORD<6) || (WORD>15) ) {
    printf ("\n K = [6, 15]\n");
    exit(0);
  }
  ...
  //========================================================================
  //**********  Reservo memoria para las READs 
  ...
  //**********  Reservo memoria para fichero FASTA
  ss0 =  (char *) malloc(nbytes1 * sizeof(char));
  if (ss0== NULL){
    printf("\n memory allocation failed in fasta File MEMORY.\n");
    exit(EXIT_FAILURE);
  }
  ...
  // ---------> New data structure to sequences of READS_file:
  ...
  // **************************************************************************
  // ******  LEO FICHERO_READS   y genero tabla de frecuencias   **************

  printf("\n\n-->>  ANALISIS  Fichero FASTA  . . .  \n"); 
  // fflush(stdin); getchar();
  long1=0;                           // icincializo tamanyo FILE-1
  secuencia = 0;  //--> comenzamos con secuencia 1: Fichero FASTA

  while (!feof(f_fa)) {
    //----->> Leo primera linea:
    ch = fgetc(f_fa);
    if(ch!=MAYOR){
      printf("\n\n----> Empezamos mal: FIN DEL ANALISIS\n");  
      break;
    }          
              
    len2=0; //--> leo nombre de la primera secuencia (FASTA)         
    do {    
      ch = fgetc(f_fa);
      ss2[len2]=ch;
      len2++;
...
//----->> Leo SEGUNDA linea: SECUENCIA DE NUCLEOTIDOS      
   len1=0;  
   ch = fgetc(f_fa); //--> leo primer nucleotido!!    
   do {
...
//----->> anotamos en nueva estructura de datos:
  ...
  if (p_l_sss->sequence){
     ...
     for (i=0; i<(aux); i++)  p_l_sss->sequence[i]= ss0[i]; 
 ...
 // fflush(stdin); getchar();
  ...
  printf("\n\n-->>  ANALISIS  Fichero READS (FASTQ)  . . . ");
  // fflush(stdin); getchar();
  long1=0;                           // icincializo tamanyo FILE-1
  secuencia = 1;  //--> comenzamos con secuencia 2
  ...
  printf("\n Llamo a wc_compare_by_index_dna \n");

  fflush(stdin); getchar();

  wc_compare_by_index_dna(index1, index2, wc);
...
void  veo_syntax(char *ss){
  printf("\n==========================================================\n");
  printf("%s  FASTA_File READS_File  Out_File  K ", ss);
  printf("\n-----------\n  k       : Word Size.");
  printf("\n==========================================================\n");
